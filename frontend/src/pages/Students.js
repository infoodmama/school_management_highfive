...
import { Plus, Upload, Download, Search, Edit, Trash2, TrendingUp, Filter } from 'lucide-react';
  ...
  const handleAddStudent = async (e) => {
    ...
    try {
      await api.createStudent({
        ...formData,
        feeTerm1: parseFloat(formData.feeTerm1),
        feeTerm2: parseFloat(formData.feeTerm2),
        feeTerm3: parseFloat(formData.feeTerm3),
      });
    ...
    <form onSubmit={onSubmit} className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <Input data-testid="student-name-input" required value={formData.studentName} onChange={(e) => setFormData({ ...formData, studentName: e.target.value })} className="rounded-xl h-12" />
        </div>
        <div>
          <Label>Roll No *</Label>
          <Input data-testid="student-rollno-input" required value={formData.rollNo} onChange={(e) => setFormData({ ...formData, rollNo: e.target.value })} className="rounded-xl h-12" />
        </div>
        <div>
          <Label>Class *</Label>
          <Select value={formData.studentClass} onValueChange={(v) => setFormData({ ...formData, studentClass: v, section: '' })}>
        ...
        <div>
          ...
          <Select value={formData.section} onValueChange={(v) => setFormData({ ...formData, section: v })}>
            ...
            <SelectContent>
              {getSections(formData.studentClass).map((s) => <SelectItem key={s} value={s}>Section {s}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
        <div>
          <Label>Father Name *</Label>
          <Input required value={formData.fatherName} onChange={(e) => setFormData({ ...formData, fatherName: e.target.value })} className="rounded-xl h-12" />
        </div>
        <div>
          <Label>Mother Name *</Label>
          <Input required value={formData.motherName} onChange={(e) => setFormData({ ...formData, motherName: e.target.value })} className="rounded-xl h-12" />
        </div>
        <div>
          <Label>Mobile Number *</Label>
          <Input required value={formData.mobile} onChange={(e) => setFormData({ ...formData, mobile: e.target.value })} className="rounded-xl h-12" />
        </div>
        <div>
          <Label>Address *</Label>
          <Input required value={formData.address} onChange={(e) => setFormData({ ...formData, address: e.target.value })} className="rounded-xl h-12" />
        </div>
        <div>
          <Label>Fee Term 1 *</Label>
          <Input type="number" required value={formData.feeTerm1} onChange={(e) => setFormData({ ...formData, feeTerm1: e.target.value })} className="rounded-xl h-12" />
        </div>
        <div>
          <Label>Fee Term 2 *</Label>
          <Input type="number" required value={formData.feeTerm2} onChange={(e) => setFormData({ ...formData, feeTerm2: e.target.value })} className="rounded-xl h-12" />
        </div>
        <div>
          <Label>Fee Term 3 *</Label>
          <Input type="number" required value={formData.feeTerm3} onChange={(e) => setFormData({ ...formData, feeTerm3: e.target.value })} className="rounded-xl h-12" />
        </div>
      </div>
      <div className="flex justify-end gap-3 pt-4">
        <Button type="button" variant="outline" onClick={() => { setShowAddDialog(false); setShowEditDialog(false); resetForm(); }} className="rounded-xl">Cancel</Button>
        <Button data-testid="submit-student-btn" type="submit" className="bg-sky-500 hover:bg-sky-600 text-white font-bold rounded-xl">{submitLabel}</Button>
      </div>
    </form>
  );
        ...
        <div className="flex flex-wrap gap-3">
          <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
            <DialogTrigger asChild>
              <Button data-testid="add-student-btn" className="bg-sky-500 hover:bg-sky-600 text-white font-bold rounded-xl active:scale-95 transition-transform">
          ...
          <label htmlFor="bulk-upload" className="cursor-pointer">
            ...
            <div className="inline-flex items-center px-4 py-2 bg-amber-400 hover:bg-amber-500 text-slate-900 font-bold rounded-xl active:scale-95 transition-transform">
              <Upload className="w-5 h-5 mr-2" />Bulk Upload
            </div>
          </label>

          <Button data-testid="download-sample-csv" onClick={handleDownloadSample} variant="outline" className="font-bold rounded-xl">
            <Download className="w-5 h-5 mr-2" />Sample CSV
          </Button>

          <Dialog open={showPromoteDialog} onOpenChange={setShowPromoteDialog}>
            <DialogTrigger asChild>
              <Button data-testid="promote-students-btn" variant="outline" className="font-bold rounded-xl bg-amber-100 text-amber-800 hover:bg-amber-200 border-amber-200">
                <TrendingUp className="w-5 h-5 mr-2" />Promote Students
              </Button>
            </DialogTrigger>
            <DialogContent>
              ...
              <form onSubmit={handlePromote} className="space-y-4">
                <div>
                  ...
                  <Select value={promoteData.fromClass} onValueChange={(v) => setPromoteData({ ...promoteData, fromClass: v })}>
                    <SelectTrigger className="rounded-xl h-12"><SelectValue placeholder="Select From Class" /></SelectTrigger>
                ...
                <div>
                  ...
                  <Select value={promoteData.toClass} onValueChange={(v) => setPromoteData({ ...promoteData, toClass: v })}>
                    <SelectTrigger className="rounded-xl h-12"><SelectValue placeholder="Select To Class" /></SelectTrigger>
                    <SelectContent>{classes.map((c) => <SelectItem key={c.className} value={c.className}>Class {c.className}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
                <div className="flex justify-end gap-3 pt-4">
          ...
          <div>
            ...
            <Select value={filters.section || '_all'} onValueChange={(v) => setFilters({ ...filters, section: v === '_all' ? '' : v })}>
              ...
              <SelectContent>
                ...
                {getSections(filters.studentClass).map((s) => <SelectItem key={s} value={s}>Section {s}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div>
            ...
            <div className="relative">
              ...
              <Input data-testid="student-search-input" placeholder="Search by name or roll no" value={filters.search} onChange={(e) => setFilters({ ...filters, search: e.target.value })} className="rounded-xl h-12 pl-10" />
            </div>
          </div>
        </div>
      </div>
